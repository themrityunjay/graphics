...
class VolumetricTest(test_case.TestCase):
  ...
  def test_render_shape_exception_not_raised(self, *shapes):
    ...
    self.assert_exception_is_not_raised(ray_radiance.render, shapes)
  ...
  def test_render_shape_exception_raised(self, error_msg, shape):
    ...
    self.assert_exception_is_raised(ray_radiance.render, error_msg, shape)
  ...
  def test_render_jacobian_random(self):
    ...
    self.assert_jacobian_is_correct_fn(
        lambda x: ray_radiance.render(x, point_distance)[0],
        [point_values])
    self.assert_jacobian_is_correct_fn(
        lambda x: ray_radiance.render(point_values, x)[0],
        [point_distance])
  ...
  def test_render_preset(self):
    ...
    rendered_image, *_ = ray_radiance.render(image_rays, distances)
